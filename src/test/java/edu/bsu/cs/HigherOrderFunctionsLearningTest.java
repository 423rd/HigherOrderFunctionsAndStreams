...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testCountByDate() {
        ...
        long actual = input
                .filter(t -> t.timestamp.isBefore(instant))
                .count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountWhitelisted() {
        ...
        long actual = input
                .filter(u -> whitelist.contains(u.user))
                .count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountNonBots() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input
                .filter(r ->! r.user.toUpperCase().contains("BOT"))
                .count();
        int expected = 26;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesInFebruary() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input
                .filter(c -> c.timestamp.atZone(ZoneId.of("UTC")).getMonth() == Month.FEBRUARY)
                .count();
        int expected = 9;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesByMonth() {
        ...
        Map<Month, Long> actual = input
                .collect(Collectors.groupingBy(c -> c.timestamp.atZone(ZoneId.of("UTC")).getMonth(), Collectors.counting()));
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = Collections.max(input
                .collect(Collectors.groupingBy(c -> c.timestamp.atZone(ZoneId.of("UTC")).getMonth(), Collectors.counting()))
